...
def algorithm(K:int, all_orders:list[Order], all_riders:list[Rider], dist_mat:np.ndarray[np.int32], timelimit=60, hparam=None)->list[str, list[int], list[int]]:
    ...
    for i in range(len(timelist)):
        ...
        if verbose>= 2 :
            print_parameters(ALNS_TIME, annealer_modify_ratio, seed, sol_edge, sol_rider, init_bundle, orders, walk_T, walk_info, bike_T, bike_info, car_T, car_info, riders_available, dist_mat, hparam, use_power, verbose)
...
def print_parameters(ALNS_TIME, annealer_modify_ratio, seed, sol_edge, sol_rider, init_bundle, orders, walk_T, walk_info, bike_T, bike_info, car_T, car_info, riders_available, dist_mat, hparam, use_power, verbose):
    print(f"ALNS_TIME: {ALNS_TIME}")
    print(f"annealer_modify_ratio: {annealer_modify_ratio}")
    print(f"seed: {seed}")
    print(f"sol_edge: {np.array(sol_edge)}")
    print(f"sol_rider: {np.array(sol_rider)}")
    print(f"init_bundle: {init_bundle}")
    print(f"orders: {np.array(orders)}")
    print(f"walk_T: {np.array(walk_T)}")
    print(f"walk_info: {walk_info}")
    print(f"bike_T: {np.array(bike_T)}")
    print(f"bike_info: {bike_info}")
    print(f"car_T: {np.array(car_T)}")
    print(f"car_info: {car_info}")
    print(f"riders_available: {riders_available}")
    print(f"dist_mat: {np.array(dist_mat)}")
